...
`)
	ini, err := parseIni(filepath.Join(dir, "main.ini"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := parseIni(filepath.Join(dir, "missing.ini")); err == nil {
...
func TestInstanceFromIniUnreadable(t *testing.T) {
	inst := Instance{PID: 42, Reason: "/proc"}
...
func TestPgbIniFromArgv(t *testing.T) {
	cases := []struct {
		...
		{[]string{"pgbouncer", "-v"}, defaultIni},
		{[]string{"pgbouncer"}, defaultIni},
	}
	for _, c := range cases {
		if got := iniFromArgv(c.argv); got != c.want {
			t.Errorf("iniFromArgv(%v) = %q, want %q", c.argv, got, c.want)
		}
	}
}
...
func TestParsePgBouncerTarget(t *testing.T) {
	...
	inst, ok = parseTarget("10.0.0.5:6433")
	if !ok || inst.ListenAddr != "10.0.0.5" || inst.ListenPort != 6433 || inst.Name != "10.0.0.5:6433" {
		t.Errorf("host:port target: ok=%v %+v", ok, inst)
	}
	inst, ok = parseTarget("pooler.example")
	if !ok || inst.ListenAddr != "pooler.example" || inst.Port() != defaultPort {
...
func TestInstanceTargetFallsBackToTCP(t *testing.T) {
	inst := Instance{SocketDir: t.TempDir(), ListenAddr: "*", ListenPort: 6432}
	host, port, unix := inst.Target()
...
func TestPoolTotalsSkipsConsolePool(t *testing.T) {
	num := func(n float64) diagres.Cell { return diagres.Cell{Display: "x", Num: n, HasNum: true} }
	res := &diagres.Result{
		Columns: []diagres.Column{{Name: "database"}, {Name: "cl_active"}, {Name: "cl_waiting"}, {Name: "sv_active"}, {Name: "sv_idle"}, {Name: "maxwait"}, {Name: "maxwait_us"}},
		Rows: [][]diagres.Cell{
...
func TestIsPgbAuthErr(t *testing.T) {
	if !isAuthErr(&pgconn.PgError{Code: "28P01", Message: "password authentication failed"}) {
		t.Errorf("28P01 is an auth error")
	}
	if !isAuthErr(errors.New("FATAL: SASL authentication failed")) {
		t.Errorf("SASL failure text is an auth error")
	}
	if isAuthErr(errors.New("dial unix /run/x: connect: no such file or directory")) {
...
func TestPgbShowAllowlist(t *testing.T) {
	...
	if _, err := c.Show(t.Context(), Instance{}, "pause"); err == nil || !strings.Contains(err.Error(), "not supported") {
