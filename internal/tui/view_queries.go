...
// ? overlay: the interval (e.g. "2s"), "paused" when toggled off at runtime, or
// "off" when disabled by config (--queries-refresh 0).
func (m *Model) refreshLabel() string {
	switch {
	case m.statRefresh <= 0:
		return "off"
	case m.statPaused:
		return "paused"
	default:
...
func (m *Model) refreshSentence() string {
	if m.statRefresh <= 0 {
		return "Auto-refresh is off (--queries-refresh 0); press t to start re-sampling."
	}
	return "It re-samples every " + m.statRefresh.String() + " — press t to pause/resume."
}
...
func (m *Model) renderStatementsInfo(height int) string {
	...
	col("T", "command type: S select · I insert · U update · D delete · M merge · T truncate · W with")
